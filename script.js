document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- STATE & LOCAL STORAGE ---
    let books = [], users = [], rentals = [], currentUser = null;
    const rewardsList = [
    ...
    const renderMyBooks = () => {
        ...
        document.getElementById('my-books-content').innerHTML = myActiveRentals.map(rental => {
            ...
            return `<div class="text-sm cursor-pointer hover:bg-white p-2 rounded-lg transition-colors" data-book-id="${book.id}"><p class="font-semibold truncate">${book.title}</p><p class="${status.class}">${status.text}</p></div>`;
        }).join('');
    };
    ...
    const renderRewards = () => {
        ...
        storeDiv.innerHTML = rewardsList.map(reward => {
            ...
            let button;
            if (hasReward) {
                button = `<button data-action="use-reward" data-reward-id="${reward.id}" class="mt-2 w-full text-sm bg-green-500 text-white py-1 px-2 rounded-md hover:bg-green-600 transition-all transform active:scale-95">Usar</button>`;
            } else {
                button = `<button ${canAfford ? '' : 'disabled'} data-action="redeem-reward" data-reward-id="${reward.id}" class="mt-2 w-full text-sm ${canAfford ? 'bg-teal-500 hover:bg-teal-600' : 'bg-gray-400 cursor-not-allowed'} text-white py-1 px-2 rounded-md transition-all transform active:scale-95">Resgatar</button>`;
            }
    ...
    const setupEventListeners = () => {
        ...
        bookList.addEventListener('click', (e) => {
            ...
            if (card) openBookModal(Number(card.dataset.bookId));
        });
        
        document.getElementById('my-books-content').addEventListener('click', (e) => {
            const card = e.target.closest('[data-book-id]');
            if (card) openBookModal(Number(card.dataset.bookId));
        });
        ...
        bookModal.addEventListener('click', (e) => {
            ...
            if (!target) return;
            const { action, bookId, tier, days, rating, target: modalTarget } = target.dataset;
            if (action === 'close-modal') closeModal(modalTarget);
            if (action === 'rent-book') rentBook(Number(bookId), tier, Number(days));
            if (action === 'return-book') returnBook(Number(bookId));
            if (action === 'rate-book') rateBook(Number(bookId), Number(rating));
        });
        ...
        document.getElementById('rewards-store-content').addEventListener('click', (e) => {
            const target = e.target.closest('[data-action]');
            if (!target) return;
            const { action, rewardId } = target.dataset;
            if (action === 'redeem-reward') redeemReward(Number(rewardId));
            if (action === 'use-reward') useReward(Number(rewardId));
        });
        
        const dbTabsContainer = document.getElementById('db-tabs-container');
        dbTabsContainer.addEventListener('click', (e) => {
            const tab = e.target.closest('.db-tab');
            if (!tab) return;
            
            dbTabsContainer.querySelectorAll('.db-tab').forEach(t => t.classList.remove('text-teal-600', 'border-teal-600', 'font-semibold') || t.classList.add('text-gray-500'));
